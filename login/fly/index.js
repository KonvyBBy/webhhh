...
const OWNER_ID = process.env.OWNER_ID || "1421909779583996025";
...
client.on("messageCreate", async (message) => {
  ...
  const embed = new EmbedBuilder()
    .setColor(0x5865f2).setTitle("🎮 Konvy Accounts")
    .setDescription("Click the button below to get your one-time login code.")
    .setFooter({ text: "Code expires in 5 minutes" });
  const row = new ActionRowBuilder().addComponents(
    new ButtonBuilder().setCustomId("get_login_code").setLabel("🔑 Get Login Code").setStyle(ButtonStyle.Primary)
  );
...
client.on("interactionCreate", async (interaction) => {
  ...
  try {
    const res = await axios.post(`${SITE_URL}/api/discord/generate-code`, {
      ...
      avatar_url: interaction.user.displayAvatarURL({ size: 128 }), secret: SECRET,
    });
    const code = res.data.code;
    if (!code) throw new Error("No code");
    await interaction.user.send(`🔑 **Your login code:** ${code}\n\nEnter it at ${SITE_URL}/login-code`);
    await interaction.editReply({ content: "✅ Code sent!" });
  } catch (err) {
